...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    else
    {
        ...
        RoutingTableEntry newEntry(/*dev=*/dev,
                                   /*dst=*/dst,
                                   /*vSeqNo=*/false, //eta bujhay oi rt entry te dest jeta setate jete valid seq ase kina
                                   /*seqNo=*/0,
...
{
    Ipv4Address origin=toOrigin.GetDestination();
    NS_LOG_FUNCTION(this << origin);

    // A node SHOULD NOT originate more than REVREQ_RATELIMIT RREQ messages per second.
    if (m_rreqCount == m_rreqRateLimit)
    {
        Simulator::Schedule(m_rreqRateLimitTimer.GetDelayLeft() + MicroSeconds(100),
    ...
    // Create REVREQ header
    RevRreqHeader revReqHeader;
    revReqHeader.SetOrigin(origin);
    //sethop->max hops
    ...
    // Using the Hop field in Routing Table to manage the expanding ring search
    uint16_t ttl = m_ttlStart;
    if (m_routingTable.LookupRoute(origin, rt))
    {
        if (rt.GetFlag() != IN_SEARCH)
        {
            ttl = std::min<uint16_t>(rt.GetHop() + m_ttlIncrement, m_netDiameter);
        ...
        if (rt.GetValidSeqNo())
        {
            revReqHeader.SetDstSeqno(rt.GetSeqNo());
        }
        else
        {
    ...
    revReqHeader.SetDstSeqno(m_seqNo);
    revReqHeader.SetUnknownSeqno(false);
    revReqHeader.SetDst(rreqHeader.GetDst());
    m_requestId++;
    revReqHeader.SetId(m_requestId);
    revReqHeader.SetLifeTime(m_myRouteTimeout);
    revReqHeader.SetHopCount(0);
    



    // Send REV_REQ as subnet directed broadcast from each interface used by raodv
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        Ptr<Socket> socket = j->first;
        Ipv4InterfaceAddress iface = j->second;

        revReqHeader.SetDst(iface.GetLocal());
        m_rreqIdCache.IsDuplicate(iface.GetLocal(), m_requestId);
        ...
        NS_LOG_DEBUG("Send RREQ with id " << revReqHeader.GetId() << " to socket");
        m_lastBcastTime = Simulator::Now();
        Simulator::Schedule(Time(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10))),
                            &RoutingProtocol::SendTo,
                            this,
...
RoutingProtocol::RecvRevRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    RevRreqHeader revReqHeader;
    p->RemoveHeader(revReqHeader);
    Ipv4Address dst = revReqHeader.GetDst();
    NS_LOG_LOGIC("RREP destination " << dst << " RREP origin " << revReqHeader.GetOrigin());

    uint32_t id = revReqHeader.GetId();
     ...
     * RREQ.
     */
    // Ipv4Address org=revReqHeader.GetOrigin();
    if (m_rreqIdCache.IsDuplicate(dst, id))
    {
        NS_LOG_DEBUG("Ignoring RREQ due to duplicate");
        return;
    }

    uint8_t hop = revReqHeader.GetHopCount() + 1;
    revReqHeader.SetHopCount(hop);

    // If RREP is Hello message
    if (dst == revReqHeader.GetOrigin())
    {
        // ProcessHello(revReqHeader, receiver);
        return;
    }
    ...
    RoutingTableEntry newEntry(
        ...
        /*dst=*/dst,
        /*vSeqNo=*/true,
        /*seqNo=*/revReqHeader.GetDstSeqno(),
        /*iface=*/m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
        /*hops=*/hop,
        /*nextHop=*/sender,
        /*lifetime=*/revReqHeader.GetLifeTime());
    RoutingTableEntry toDst;
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        ...
        if (
            ...
            ((int32_t(revReqHeader.GetDstSeqno()) - int32_t(toDst.GetSeqNo())) > 0) ||

            // (iii) the sequence numbers are the same, but the route is marked as inactive.
            (revReqHeader.GetDstSeqno() == toDst.GetSeqNo() && toDst.GetFlag() != VALID) ||

            // (iv) the sequence numbers are the same, and the New Hop Count is smaller than the
            // hop count in route table entry.
            (revReqHeader.GetDstSeqno() == toDst.GetSeqNo() && hop < toDst.GetHop()))
        {
            m_routingTable.Update(newEntry);
        }
    }
    ...
    // if (revReqHeader.GetAckRequired())
    // {
    //     SendReplyAck(sender);
    //     revReqHeader.SetAckRequired(false);
    // }
    NS_LOG_LOGIC("receiver " << receiver << " origin " << revReqHeader.GetOrigin());
    if (IsMyOwnAddress(revReqHeader.GetOrigin()))
    {
    ...
    RoutingTableEntry toOrigin;
    if (!m_routingTable.LookupRoute(revReqHeader.GetOrigin(), toOrigin) ||
    ...
    // else
    // {
    //     if (toOrigin.GetNextHop() == sender)
    //     {
    //         NS_LOG_DEBUG("Drop RREQ from " << sender<< ", dest next hop " << toDst.GetNextHop());
    //         return;
    //     }
    // }
    ...
    // if (m_routingTable.LookupValidRoute(revReqHeader.GetDst(), toDst))
    // {
