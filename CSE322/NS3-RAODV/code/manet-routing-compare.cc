...
class RoutingExperiment
{
  ...
  private:
    ...
    uint32_t m_nNodes{40};
    uint32_t m_packetsPerSecond{100};
    double m_nodeSpeed{5.0};
    double m_simTime{30.0};
    int m_packetSize{64};
    std::string m_dataFile{"output.dat"};
};
...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    cmd.AddValue("protocol", "Routing protocol (OLSR,AODV,RAODV, DSDV, DSR)", m_protocolName);
    cmd.AddValue("flowMonitor", "enable FlowMonitor", m_flowMonitor);
    
    cmd.AddValue("nNodes", "Number of nodes", m_nNodes);
    cmd.AddValue("pps", "Packets per second", m_packetsPerSecond);
    cmd.AddValue("nodeSpeed", "Node speed in m/s", m_nodeSpeed);
    cmd.AddValue("dataFile","Plotting File", m_dataFile);
    cmd.AddValue("simTime","Simulation Time", m_simTime);
    cmd.Parse(argc, argv);


    std::vector<std::string> allowedProtocols{"OLSR","AODV","RAODV", "DSDV", "DSR"};

    if (std::find(std::begin(allowedProtocols), std::end(allowedProtocols), m_protocolName) ==
        std::end(allowedProtocols))
    {
        std::cout<<m_protocolName<<" ffff \n";
...
RoutingExperiment::Run()
{
    ...
    int nWifis = m_nNodes;
    m_nSinks=std::max(1,nWifis/2);

    double TotalTime = m_simTime;
    double bitRate=m_packetsPerSecond*m_packetSize*8;
    
    std::string rate(std::to_string(bitRate));
    ...
    for (int i = 0; i < m_nSinks; i++)
    {
        ...
        temp.Start(Seconds(1)); //var->GetValue(1.0, 2.0)));
    ...
    if (m_flowMonitor)
    {
        ...
        for (auto &flow : stats) {
            totalThroughput += (flow.second.rxBytes * 8.0) / (flow.second.timeLastRxPacket.GetSeconds() - flow.second.timeFirstTxPacket.GetSeconds())/1024.0;
        ...
        double delivaryratio = (double)totalRxPackets / totalTxPackets * 100;
        double dropRatio = (double)totalLostPackets / totalTxPackets * 100;

        std::string plotFilename = m_dataFile;
                            // "results_" + std::to_string(m_nNodes) + "_" + 
                            // std::to_string(m_packetsPerSecond) + "_" + 
                            // std::to_string(m_nodeSpeed) + ".txt";
        
        std::ofstream outPlotFile(plotFilename,std::ofstream::app);
        outPlotFile<< m_protocolName <<","<< m_nNodes <<","<< m_packetsPerSecond << ","<< m_nodeSpeed << "," <<avgThroughput << "," << avgDelay << "," << delivaryratio << "," << dropRatio << "\n";
        outPlotFile.close();
    }
    Simulator::Destroy();
}
