...
class TcpHybla : public TcpNewReno
{
  ...
  private:
    ...
    //bandwidth estimate
    double m_bwe;          // Current bandwidth estimate
    Time m_lastBweUpdateTime; // Last time BWE was updated
    uint32_t m_bytesAcked;   // Bytes acked since last BWE update
    uint32_t m_channelCapacity; // Link capacity in bps
  ...
  private:
     ...
     */
    void RecalcParam(const Ptr<TcpSocketState>& tcb);
    void UpdateBandwidthEstimate(Ptr<TcpSocketState> tcb, uint32_t bytesAcked);
    uint32_t CalculateWindow (Ptr<TcpSocketState> tcb);
